...
## Define Parameters
which_dataset = 6; # Abdominal Maps 1-6 (only 3 and 4 were made open source in TCI paper)
...
fTx = 8.0e6; # frequency [Hz]
fBW = 12.0e6; # bandwidth at half maximum [Hz]
...
max_iterations = 15; # Number of Iterations of Conjugate Gradient
useLineSearch = True; # use line search (true); linearized step size (false)
max_iterations_golden_section = 9; # maximum iterations of golden-section search
...
if useDistortionMatrixMethod:
    ...
    roundoff_scaling = 1e0; # Scaling Residuals to Avoid Over/Underflow Errors (e.g., Step Size Formula) 
...
dBrange = 40; # Dynamic Range [decibels]
rng_min = 1400; rng_max = 1650; # Sound Speed Display Range [m/s]

# Load File and Original k-Wave Simulation Grid
load_filename = '../kWaveSims/datasets/AbdominalMap'+str(which_dataset)+'.mat'; 
...
scat = dataset['scat']
del dataset; 
xorig, zorig = np.copy(x), np.copy(z); 
X, Z = np.meshgrid(xorig, zorig); 
...
# Interpolate from True Sound Speed
xrecon, zrecon = x, (z[:-1]+z[1:])/2; 
Xrecon, Zrecon = np.meshgrid(xrecon, zrecon); 
# Equivalent to: clayers = interp2d(xorig, zorig, C, kind='cubic', bounds_error=False, fill_value=cbfm)(xrecon, zrecon)
clayers_true = RegularGridInterpolator((zorig, xorig), C, 
    method='cubic', bounds_error=False, fill_value=cbfm)((Zrecon, Xrecon)); 
clayers_const = cbfm*np.ones((z.size-1, x.size)); 
sLayers_true, sLayers_const = 1/clayers_true, 1/clayers_const; 
...
# Reconstruct Image Using True Sound Speed
start = timer.time(); 
img_recon_true = img_recon_fast(sLayers_true); 
end = timer.time(); 
print(f"B-Mode Image (True Sound Speed) Execution time: {end - start:.6f} seconds")
...
for iteration in range(max_iterations):
    ...
    imagesc(x, z[1::], clayers_const, rng=[rng_min, rng_max], numticks=(0, 0), cmap='gray'); 
    plt.xlabel('X-Coordinate of Screen [m]'); 
    plt.ylabel('Z-Coordinate of Screen [m]'); 
    plt.title('Constant Sound Speed [m/s]'); 
    plt.colorbar();
    # Plot True Sound Speed
    plt.subplot2grid((2, 3), (0, 1)); 
    imagesc(x, z[1::], clayers_true, rng=[rng_min, rng_max], numticks=(0, 0), cmap='gray'); 
    plt.xlabel('X-Coordinate of Screen [m]'); 
    plt.ylabel('Z-Coordinate of Screen [m]'); 
    plt.title('True Sound Speed [m/s]'); 
    plt.colorbar();
    # Plot Estimated Sound Speed - Based on Estimated Aberration Delays
    clayers_estim = 1/sLayers_estim; # sound speed [m/s]
    plt.subplot2grid((2, 3), (0, 2)); 
    imagesc(x, z[1::], clayers_estim, rng=[rng_min, rng_max], numticks=(0, 0), cmap='gray'); 
    plt.xlabel('X-Coordinate of Screen [m]'); 
    plt.ylabel('Z-Coordinate of Screen [m]'); 
    plt.title('Estimated Sound Speed [m/s]'); 
    plt.colorbar(); 
    # Plot Image
    ...
    imagesc(x, z[1::], dispImage(img_recon_true), rng=[-dBrange,0], numticks=(0, 0), cmap='gray'); 
    plt.title('Ideal Corrected B-Mode Image'); 
    plt.xlabel('x [m]'); plt.ylabel('z [m]'); 
    ...
    # Save Results
    plt.savefig('../Figures/AbdominalMaps/Map'+str(which_dataset)+'/Recon'+str(iteration)+'.png'); 
...
data_to_save = {
    'cLayers_true': clayers_true, 
    'cLayers_estim_history': cLayers_estim_history,
    'img_recon_true': img_recon_true,
    'img_recon_history': img_recon_history,
    'x': x, 'z': z[1::]
}
savemat('../Figures/AbdominalMaps/Map'+str(which_dataset)+'/FullHistory.mat', data_to_save); 
