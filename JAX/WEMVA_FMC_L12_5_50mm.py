...
## Define Parameters
which_dataset = 4; # 1 for dataset released in TCI paper
                   # 2 for symmetric aberrator - point targets but no hypoechoic lesions
                   # 3 for asymmetric aberrator - same as used in TCI paper 
                   # 4 for asymmetric aberrator - slightly different slice from TCI paper (used in graphical abstract for TUSON paper)
# Dataset-Specific Parameter Choices
if which_dataset == 1: 
    dov = 44e-3; # Max Depth [m] -- 44 mm for datasets 1 and 2
    ...
    dov = 44e-3; # Max Depth [m] -- 44 mm for datasets 1 and 2
    max_iterations = 30; # Number of Iterations of Conjugate Gradient
    layerPrior = 0.9; # Ranging from 0 (no prior) to 1 (fully layered)
elif which_dataset == 3:
    dov = 49e-3; # Max Depth [m] -- 49 mm for datasets 3 and 4
    max_iterations = 30; # Number of Iterations of Conjugate Gradient
    layerPrior = 0.99; # Ranging from 0 (no prior) to 1 (fully layered)
elif which_dataset == 4: # (used in graphical abstract for TUSON paper)
    dov = 49e-3; # Max Depth [m] -- 49 mm for datasets 3 and 4
    max_iterations = 50; # Number of Iterations of Conjugate Gradient
    layerPrior = 0.993; # Ranging from 0 (no prior) to 1 (fully layered)
# Universal Parameters Below
upsamp_x = 1; # Upsampling in x (assuming dx = pitch) - must be integer
upsamp_z = 0.5; # Upsampling in z (assuming dz = lambda/2) - can be fractional
Nx0 = 320; # Number of Points Laterally in x
cbfm = 1460; # initial sound speed [m/s]
fTx = 8.0e6; # frequency [Hz]
fBW = 9.0e6; # bandwidth at half maximum [Hz]
pulse_cutoff = 1e-3; # passband cutoff
ord = 100; # Strength of Anti-Aliasing Window 
# Sound Speed Reconstruction Parameters
z_idx_start = 10; # Depth Start Index to Start Minimizing Residuals
depthCorrection = True; # Whether or Not to Perform Depth-Ambiguity Correction
roundoff_scaling = 1e-10; # Scaling Residuals to Avoid Over/Underflow Errors (e.g., Step Size Formula) 
...
blur_z_FWHM_wvlens = 36; # blurring length over z [in wavelengths]
blur_x_FWHM_wvlens = 36; # blurring length over x [in wavelengths]
...
FWHM_SubsurfaceOffset_Wvlens = 18; # Full-Width at Half-Max (FWHM) in Subsurface Offset [in Wavelengths]
...
dBrange = 80; # Dynamic Range [decibels]
rng_min = 1350; rng_max = 1650; # Sound Speed Display Range [m/s]
...
TGC = (z[1::,np.newaxis]**2 + (np.max(z)/4)**2); 
...
plt.ion(); plt.figure(figsize=(13,9))
for iteration in range(max_iterations):
    ...
    plt.xlabel('x [m]'); plt.ylabel('z [m]'); plt.title('Estimated Sound Speed + Corrected B-Mode');     
    plt.show(); plt.pause(1); 
    # Save Data / Figures
    if which_dataset == 1: # 1 for dataset released in TCI paper
        plt.savefig('../Figures/Phantom/PhantomL12-5-50mm/Recon'+str(iteration)+'.png'); 
    elif which_dataset == 2: # 2 for symmetric aberrator - point targets but no hypoechoic lesions
...
if which_dataset == 1: # 1 for dataset released in TCI paper
    savemat('../Figures/Phantom/PhantomL12-5-50mm/FullHistory.mat', data_to_save); 
elif which_dataset == 2: # 2 for symmetric aberrator - point targets but no hypoechoic lesions
